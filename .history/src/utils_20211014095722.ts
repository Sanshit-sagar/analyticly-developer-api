...
const parseStringField = (fieldName: string, stringField: unknown): string => {
    if(!stringField || !isString(stringField)) {
        throw new Error(`Incorrect or missing ${fieldName}: ${stringField}`)
    }
    return stringField
}
...
const toNewSlug = ({ slug, expiration, password, comment }: Fields): NewSlug => {
    const newSlug: NewSlug = {
        slug: parseStringField('slug', slug)
